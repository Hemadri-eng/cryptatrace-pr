...
export default function CaseDetail() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {caseData.risk_score == null && (
          <button
            ...
            className="px-4 py-2 rounded-md bg-cyan-500 hover:bg-cyan-400 text-ink-950 text-sm font-semibold"
          >
            Submit &amp; Investigate
          </button>
        )}
      ...
      {tab === "Fund Flow" && (
        investigation ? <FundFlowGraph graphData={investigation.graph_data} /> : <EmptyState text="No fund-flow data yet — run an investigation first." />
      )}

      {tab === "Evidence" && (
        evidence.length > 0 ? <EvidenceTab evidence={evidence} /> : <EmptyState text="No evidence recorded yet." />
      )}

      {tab === "Timeline" && (
        investigation ? <TimelineTab timeline={investigation.timeline} /> : <EmptyState text="No timeline available yet." />
      )}
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="lg:col-span-2 bg-white rounded-lg border border-slate-200 p-5 space-y-4">
        ...
        <p className="text-sm text-ink-700">{caseData.description || "No description provided."}</p>
        <div className="grid grid-cols-2 gap-4 text-sm pt-2">
          ...
          <Field label="Amount" value={caseData.approx_amount ? `${caseData.approx_amount} ${caseData.currency}` : "—"} />
        ...
        {caseData.victim_notes && (
          <div className="pt-2">
            ...
            <p className="text-sm text-ink-700 bg-slate-50 rounded-md p-3">{caseData.victim_notes}</p>
          </div>
        )}
      </div>
      <div className="bg-white rounded-lg border border-slate-200 p-5 space-y-3">
        <h2 className="text-sm font-semibold text-ink-900">Reported Wallet / Tx</h2>
        <p className="font-mono text-xs break-all text-ink-700 bg-slate-50 rounded-md p-3">
          {caseData.wallet_address || caseData.transaction_hash}
        </p>
        {investigation && (
          <>
            <h2 className="text-sm font-semibold text-ink-900 pt-2">Probable Destination</h2>
            <p className="text-sm text-ink-900 font-medium">{investigation.probable_entity_name}</p>
            <p className="text-xs text-ink-600/70">Confidence: {investigation.probable_entity_confidence}%</p>
          </>
        )}
      </div>
    </div>
...
function InvestigationTab({ investigation, report, generatingReport, onGenerateReport }) {
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-display font-bold" style={{ color: riskColor(investigation.risk_level) }}>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-ink-950 text-sm font-semibold px-4 py-2.5 rounded-md disabled:opacity-60"
          >
      ...
      {report && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-sm">
          Report <span className="font-mono">{report.report_number}</span> generated.
          <button onClick={() => openReport(report)} className="text-emerald-700 font-medium ml-1 hover:underline">
            View report →
          </button>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5">
        <h2 className="text-sm font-semibold text-ink-900 mb-2">Executive Summary</h2>
        <p className="text-sm text-ink-700 leading-relaxed">{investigation.executive_summary}</p>
      </div>

      <div className="bg-white rounded-lg border border-slate-200 p-5">
        <h2 className="text-sm font-semibold text-ink-900 mb-4">Risk Score Breakdown</h2>
        <div className="space-y-3">
          {investigation.risk_factors.map((f, i) => (
            <div key={i}>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-ink-900">{f.label}</span>
                <span className="font-mono text-cyan-600">+{f.weight}</span>
              </div>
              <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-cyan-500 rounded-full" style={{ width: `${(f.weight / 20) * 100}%` }} />
              </div>
              <p className="text-xs text-ink-600/60 mt-1">{f.detail}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <h2 className="text-sm font-semibold text-ink-900 mb-3">Key Findings</h2>
        <div className="grid sm:grid-cols-2 gap-3">
          {investigation.findings.map((f, i) => {
            const Icon = FINDING_ICONS[f.title] || AlertTriangle;
            const sevColor = { low: "text-emerald-600", medium: "text-amber-600", high: "text-orange-600", critical: "text-red-600" }[f.severity] || "text-ink-600";
            return (
              <div key={i} className="bg-white rounded-lg border border-slate-200 p-4">
                ...
                <p className="text-xs text-ink-600/70 mb-2">{f.description}</p>
                <span className="text-xs font-mono text-cyan-600">Risk contribution: +{f.risk_contribution}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-5">
        <h2 className="text-sm font-semibold text-ink-900 mb-2">Probable Exchange</h2>
        <p className="text-sm font-medium text-ink-900">{investigation.probable_entity_name}</p>
        <p className="text-xs text-ink-600/70 mt-0.5 mb-2">Confidence: {investigation.probable_entity_confidence}%</p>
        <p className="text-sm text-ink-700">{investigation.probable_entity_reasoning}</p>
      </div>
    </div>
  );
}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-ink-900">Transaction Detail — {selected.evidence_code}</h3>
              <button onClick={() => setSelected(null)} className="text-ink-600/50 hover:text-ink-900 text-lg leading-none">×</button>
            </div>
            <p className="text-[11px] text-ink-600/50 bg-slate-50 rounded px-3 py-2 font-mono break-all">
              {selected.transaction_hash}
            </p>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <ModalRow label="Blockchain" value={selected.blockchain} />
              <ModalRow label="Amount" value={`${selected.amount} ${selected.token}`} />
              <ModalRow label="Timestamp" value={formatDate(selected.timestamp)} />
              <ModalRow label="Finding supported" value={selected.finding_supported} />
            </div>
            <div>
              <p className="text-xs text-ink-600/50 mb-1">Source wallet</p>
              <p className="text-xs font-mono break-all text-ink-800">{selected.source_wallet}</p>
            </div>
            <div>
              <p className="text-xs text-ink-600/50 mb-1">Destination wallet</p>
              <p className="text-xs font-mono break-all text-ink-800">{selected.destination_wallet}</p>
            </div>
      ...
      <ol className="relative border-l border-slate-200 ml-2 space-y-6">
        {timeline.map((t, i) => (
          <li key={i} className="ml-4">
            ...
            <p className="text-xs text-ink-600/60">{formatDate(t.timestamp)}</p>
            <p className="text-sm text-ink-900 font-medium">{t.event}</p>
            <p className="text-xs text-ink-600/70">{t.detail}</p>
            {t.tx_hash && <p className="text-[11px] font-mono text-cyan-600 mt-0.5">{truncateAddr(t.tx_hash, 12)}</p>}
          </li>
        ))}
      </ol>
    </div>
